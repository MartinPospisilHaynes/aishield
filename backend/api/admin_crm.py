...
async def crm_factory_reset(
    ...
    Redis ARQ frontu, in-memory cache.
    ...
    settings = get_settings()
    results = {"auth": None, "db": None, "storage": None, "redis": None, "cache": None, "errors": []}

    # ── 1. Smazání VŠECH Auth uživatelů ──
    # Admin panel používá vlastní auth (username/password z .env),
    # NE Supabase Auth. Všichni Supabase Auth uživatelé = zákazníci → smazat.
    try:
        sb_url = settings.supabase_url
        sb_key = settings.supabase_service_role_key
        async with httpx.AsyncClient(timeout=30.0) as client:
            resp = await client.get(
                f"{sb_url}/auth/v1/admin/users?per_page=500",
            ...
            for u in users:
                ...
                if del_resp.status_code < 300:
                    deleted_users += 1
            results["auth"] = f"Smazáno {deleted_users}/{len(users)} Supabase Auth uživatelů"
    except Exception as e:
        results["auth"] = f"CHYBA: {e}"
        results["errors"].append(f"auth: {e}")

    # ── 2. Výmaz VŠECH DB tabulek (FK pořadí) ──
    try:
        import os
        db_pass = os.environ.get("SUPABASE_DB_PASSWORD", "")
        sb_url_raw = settings.supabase_url
        ...
        # KOMPLETNÍ seznam VŠECH tabulek včetně těch co chyběly
        layers = [
            # Layer 1: Leaf tables (no dependents)
            ["findings", "questionnaire_responses", "documents", "alerts", "scan_diffs",
             "chat_messages", "chat_feedback", "company_activities", "data_access_log",
             "orchestrator_log", "mart1n_conversations", "engine_health_counters"],
            # Layer 2: Payment tables
            ["subscription_payments", "invoices", "orders", "subscriptions", "payments"],
            # Layer 3: Email tables
        ...
        # Catch-all: Smaž COKOLIV co zbylo (kromě ai_act_chunks)
        for t in sorted(existing):
            ...
            try:
                ...
                if cnt > 0:
                    ...
                    db_report.append(f"{t}: {cur.rowcount} (catch-all)")
            except Exception as e:
                db_report.append(f"{t}: catch-all CHYBA {e}")
                conn.rollback()
                conn.autocommit = True
        ...
        results["db"] = {
            ...
            "verification": "OK — všechny tabulky prázdné" if not non_empty else f"NEPRÁZDNÉ: {non_empty}",
        }
    except Exception as e:
    ...
    # ── 3. Storage cleanup — REKURZIVNÍ (documents + screenshots) ──
    storage_deleted = 0
    try:
        supabase = get_supabase()

        for bucket_name in ["documents", "screenshots"]:
            try:
                # List root-level items (folders = company UUIDs, or files)
                root_items = supabase.storage.from_(bucket_name).list(path="", options={"limit": 1000})
                if not root_items:
                    continue

                for item in root_items:
                    item_name = item.get("name", "")
                    item_id = item.get("id")  # Files have id, folders don't
                    ...
                    if item_id:
                        ...
                        # It's a folder — list contents and delete recursively
                        try:
                            folder_files = supabase.storage.from_(bucket_name).list(
                                path=item_name, options={"limit": 1000}
                            )
                            if folder_files:
                                file_paths = [f"{item_name}/{f['name']}" for f in folder_files if f.get("name")]
                                if file_paths:
                                    supabase.storage.from_(bucket_name).remove(file_paths)
                                    storage_deleted += len(file_paths)
                        except Exception as folder_err:
                            results["errors"].append(f"storage folder {bucket_name}/{item_name}: {folder_err}")

            except Exception as bucket_err:
                results["errors"].append(f"storage bucket {bucket_name}: {bucket_err}")

        results["storage"] = f"Smazáno {storage_deleted} souborů ze všech bucketů (documents + screenshots)"
    except Exception as e:
        results["storage"] = f"CHYBA: {e}"
        results["errors"].append(f"storage: {e}")

    # ── 4. Redis flush (ARQ job queue) ──
    try:
        import redis as _redis
        rc = _redis.Redis(host="localhost", port=6379, db=0)
        key_count = rc.dbsize()
        rc.flushdb()
        results["redis"] = f"Redis DB0 flushed ({key_count} keys smazáno)"
        logger.info(f"[FACTORY RESET] Redis flushed: {key_count} keys")
    except Exception as e:
        results["redis"] = f"CHYBA: {e}"
        results["errors"].append(f"redis: {e}")

    # ── 5. Invalidace VŠECH in-memory caches ──
    try:
        # Business overview cache
        _biz_overview_cache["data"] = None
        _biz_overview_cache["expires"] = 0

        # Any other module-level caches
        results["cache"] = "Všechny in-memory cache vymazány (_biz_overview_cache)"
        logger.info("[FACTORY RESET] In-memory caches invalidated")
    except Exception as e:
        results["cache"] = f"CHYBA: {e}"
    ...
    try:
        import shutil, glob
        temp_deleted = 0
        if os.path.isdir("/tmp/pdfs"):
            count = len(os.listdir("/tmp/pdfs"))
            shutil.rmtree("/tmp/pdfs")
            temp_deleted += count
        patterns = [
            "/tmp/*scan*.py", "/tmp/*scan*.log", "/tmp/*scan*.html",
            "/tmp/*docgen*.py", "/tmp/*docgen*.log", "/tmp/*docgen*.txt",
            "/tmp/*regen*.py", "/tmp/*compliance*", "/tmp/*transparency*",
            "/tmp/deep_scan_email_preview*",
        ]
        for pattern in patterns:
            for fpath in glob.glob(pattern):
                try:
                    ...
                    pass
        results["vps_temp"] = f"Smazáno {temp_deleted} temp souborů"
        logger.info(f"[FACTORY RESET] VPS temp cleanup: {temp_deleted} files")
    except Exception as e:
        results["vps_temp"] = f"CHYBA: {e}"
        results["errors"].append(f"vps_temp: {e}")
    ...
    return {
        ...
        "message": "Factory reset dokončen — systém je čistý" if success else "Factory reset s chybami",
        "results": results,
    }
