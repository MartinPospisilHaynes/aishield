...
async def deep_scan_job(ctx: dict, scan_id: str, url: str, company_id: str):
    ...
    try:
        ...
        # ══════════════════════════════════════════════════════════════
        # ══════════════════════════════════════════════════════════════
        # Automatické generování dokumentů je ZAKÁZÁNO.
        # Generování spouští admin ručně přes CRM po kontrole výsledků.
        # Zde pouze notifikujeme admina, že deep scan doběhl.
        # ══════════════════════════════════════════════════════════════
        try:
            # Aktualizovat workflow_status — deep scan hotov, čeká na admin akci
            supabase.table("companies").update({
                "workflow_status": "deep_scan_done",
            }).eq("id", company_id).execute()

            # Notifikovat admina — deep scan hotov, může spustit generování
            try:
                comp_name = ""
                comp_email_addr = ""
                if company.data:
                ...
                await _send(
                    to="info@aishield.cz",
                    subject=f"Deep scan hotov — {comp_name} čeká na spuštění výroby dokumentů",
                    html=(
                        f"<h2>Deep scan dokončen</h2>"
                        ...
                        f"<hr>"
                            ...
                            f"<hr>"
                            f"<p><strong>Další krok:</strong> Spusťte generování dokumentů ručně v CRM.</p>"
                            f"<p>👉 <a href='https://aishield.cz/admin/crm'>Otevřít CRM →</a></p>"
                        ),
                        from_email="info@aishield.cz",
                        from_name="AIshield.cz",
                    )
                    logger.info(f"[DeepScan] Admin notifikace odeslána — čeká na ruční spuštění generování")
                except Exception as notify_err:
                    logger.error(f"[DeepScan] Chyba admin notifikace: {notify_err}")

        except Exception as gen_err:
            logger.error(f"[DeepScan] Chyba post-scan workflow: {gen_err}", exc_info=True)
